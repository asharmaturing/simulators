...
const GuidePanel: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed right-0 top-0 bottom-0 w-96 bg-slate-900 border-l border-slate-800 shadow-2xl z-40 flex flex-col transform transition-transform duration-300 pt-16">
      {/* Header */}
      <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900">
        <div className="flex items-center gap-2">
          <Book className="text-cyan-400" size={20} />
          <h2 className="text-lg font-bold text-white">Learning Guide</h2>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {GUIDE_CONTENT.map((chapter) => (
          <div key={chapter.id} className="border border-slate-700 rounded-lg overflow-hidden bg-slate-800/30">
            <button 
            ...
            >
              ...
              {expandedSections[chapter.id] ? <ChevronDown size={16} className="text-slate-400" /> : <ChevronRight size={16} className="text-slate-400" />}
            </button>
            
            {expandedSections[chapter.id] && (
              <div className="border-t border-slate-700 bg-slate-950/50 p-4 space-y-6">
